...
class DebugNodeFactory(AbstractNodeFactory):
    def makeNode(self, tag: str, content: str = '', attributes: dict = None):
        if tag == 'html':
            print("Html node is created.")
            return Html(content, attributes)
        elif tag == 'head':
            print("Head node is created.")
            return Head(content, attributes)
        elif tag == 'title':
            print("Title node is created.")
            return Title(content, attributes)
        elif tag == 'body':
            print("Body node is created.")
            return Body(content, attributes)
        elif tag == 'div':
            print("Div node is created.")
            return Div(content, attributes)
        elif tag == 'b':
            print("B node is created.")
            return B(content, attributes)
        else:
            raise ValueError("Unknown tag")

# Main function using the DebugNodeFactory
def main():
